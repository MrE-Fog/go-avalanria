...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [avn/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    avndb.Database  // Database to state sync into (and deduplicate via)
	...
	snapSync       bool         // Whavner to run state sync over the snap protocol
	...
	// Testing hooks
	syncInitHook     func(uint64, uint64)  // Mavnod to call upon initiating a new sync run
	bodyFetchHook    func([]*types.Header) // Mavnod to call upon starting a block body fetch
	receiptFetchHook func([]*types.Header) // Mavnod to call upon starting a receipt fetch
	chainInsertHook  func([]*fetchResult)  // Mavnod to call upon inserting a chain of blocks (possibly in multiple invocations)
}
...
func New(checkpoint uint64, stateDb avndb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() avalanria.SyncProgress {
...
// Synchronising returns whavner the downloader is currently retrieving blocks.
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
	...
	if errors.Is(err, errInvalidChain) || errors.Is(err, errBadPeer) || errors.Is(err, errTimeout) ||
		...
		if d.dropPeer == nil {
			// The dropPeer mavnod is nil when `--copydb` is used for a local copy.
...
// checks fail an error will be returned. This mavnod is synchronous
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	// sync mode. Long term we could drop fast sync or merge the two togavner,
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	if p.version < avn.AVN65 {
		return fmt.Errorf("%w: advertized %d < required %d", errTooOld, p.version, avn.AVN65)
	}
	mode := d.getMode()

	log.Debug("Synchronising with the network", "peer", p.id, "avn", p.version, "head", hash, "td", td, "mode", mode)
...
// not wait for the running download goroutines to finish. This mavnod should be
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (commonAncestor uint64, err error) {
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somavning valid
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // Whavner the next request is pivot verification
...
//
// The mavnod returns the entire filled skeleton and also the number of headers
...
// types, this mavnod is used by each for data gathering and is instrumented with
...
//  - expire:      task callback mavnod to abort requests that took too long and return the faulty peers (traffic shaping)
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somavning completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somavning)
				// R: Nothing to give
				if mode != LightSync {
				...
				// peer gave us somavning useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if somavning failed in between processing chunks
				select {
				case <-d.cancelCh:
					rollbackErr = errCanceled
					return errCanceled
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The InsertChain mavnod in blockchain.go will sometimes return an out-of-bounds index,
			// when it needs to preprocess blocks to import a sidechain.
			// The importer will put togavner a new list of blocks to import, which is a superset
