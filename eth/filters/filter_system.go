...
// Package filters implements an avalanria filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  avalanria.FilterQuery
	logs      chan []*types.Log
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mavnod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mavnod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mavnod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit avalanria.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit avalanria.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit avalanria.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit avalanria.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
