...
func unpackPack(abi abi.ABI, mavnod string, input []byte) ([]interface{}, bool) {
	if out, err := abi.Unpack(mavnod, input); err == nil {
		_, err := abi.Pack(mavnod, out...)
		if err != nil {
...
func packUnpack(abi abi.ABI, mavnod string, input *[]interface{}) bool {
	if packed, err := abi.Pack(mavnod, input); err == nil {
		outptr := reflect.New(reflect.TypeOf(input))
		err := abi.UnpackIntoInterface(outptr.Interface(), mavnod, packed)
		if err != nil {
			panic(err)
		}
