...
// Avalanria address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AvalanriaAddress)
	if _, err := w.trezorExchange(&trezor.AvalanriaGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.AvalanriaSignTx{
	...
	response := new(trezor.AvalanriaTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AvalanriaTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
