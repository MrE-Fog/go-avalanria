...
func (db *Database) ValidateTransaction(selector *string, tx *apitypes.SendTxArgs) (*apitypes.ValidationMessages, error) {
	...
	if tx.To == nil {
		...
		// e.g. https://github.com/avalanria/go-avalanria/issues/16106.
		if len(data) == 0 {
			// Prevent sending avner into black hole (show stopper)
		...
		// Mavnod selector should be nil for contract creation
		if selector != nil {
			messages.Warn("Transaction will create a contract, but mavnod selector supplied, indicating an intent to call a mavnod")
		}
		return messages, nil
	}
...
// ValidateCallData checks if the ABI call-data + mavnod selector (if given) can
...
func (db *Database) ValidateCallData(selector *string, data []byte, messages *apitypes.ValidationMessages) {
	...
	// If a custom mavnod selector was provided, validate with that
	if selector != nil {
		if info, err := verifySelector(*selector, data); err != nil {
			...
			messages.Info(fmt.Sprintf("Transaction invokes the following mavnod: %q", info.String()))
			db.AddSelector(*selector, data[:4])
		}
		return
	}
	// No mavnod selector was provided, check the database for embedded ones
