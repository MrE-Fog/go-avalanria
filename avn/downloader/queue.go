...
type fetchRequest struct {
	...
	From    uint64          // [avn/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [avn/62] Requested headers, sorted by request order
...
// InFlightHeaders retrieves whavner there are header fetch requests currently
// in flight.
...
// InFlightBlocks retrieves whavner there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whavner there are receipt fetch requests currently
// in flight.
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading somavning (sanity check to
	// not corrupt state)
...
// returns a flag whavner empty blocks were queued requiring processing.
...
// also returns a flag whavner empty receipts were queued requiring importing.
...
// skipping any previously failed ones. This mavnod is a generic version used
// by the individual special reservation functions.
//
// Note, this mavnod expects the queue lock to be already held for writing. The
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
//
// Returns:
//   item     - the fetchRequest
//   progress - whavner any progress was made
//   throttle - if the caller should throttle for a while
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, kind uint) (*fetchRequest, bool, bool) {
	// Short circuit if the pool has been depleted, or if the peer's already
	// downloading somavning (sanity check not to corrupt state)
	if taskQueue.Empty() {
...
// Revoke cancels all pending requests belonging to a given peer. This mavnod is
...
//
// Note, this mavnod expects the queue lock to be already held. The
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
...
// cache. This mavnod either accepts all headers it received, or none of them
// if they do not map correctly to the skeleton.
//
// If the headers are accepted, the mavnod makes an attempt to deliver the set
...
// The mavnod returns the number of blocks bodies accepted from the delivery and
...
// The mavnod returns the number of transaction receipts accepted from the delivery
