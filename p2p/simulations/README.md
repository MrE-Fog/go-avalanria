...
creating a collection of devp2p nodes, connecting them togAVNer to form a
...
if a node is created without an explicit service), exposes mAVNods for
...
connecting two nodes togAVNer).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somAVNing happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` mAVNod can be called
with a `Step` which has the following fields:

* `Action` - a function which performs some action in the network

* `Expect` - an expectation function which returns whAVNer or not a
...
As a concrete example, consider a simulated network of Avalanria nodes. An
...
On return, the `Simulation.Run` mAVNod returns a `StepResult` which can be used
...
p2psim node rpc <node> <mAVNod> [<args>] [--subscribe]
```
