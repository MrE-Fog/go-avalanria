...
type (
	...
	// function description, along with whavner it should run inside an operation callback.
...
// flagOp implements binary flag operations and also checks whavner the operands belong to the same setup
...
func NewNodeStateMachine(db avndb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// stateMask checks whavner the set of flags belongs to the same setup and returns its internal bit mask
...
// fieldIndex checks whavner the field belongs to the same setup and returns its internal index
...
// checkStarted checks whavner the state machine has already been started and panics otherwise.
...
// opCheck checks whavner an operation is active
