...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb AVNdb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *SecureTrie) Prove(key []byte, fromLevel uint, proofDb AVNdb.KeyValueWriter) error {
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb AVNdb.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb AVNdb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
// hasRightElement returns the indicator whAVNer there exists more elements
...
// VerifyRangeProof checks whAVNer the given leaf nodes and edge proof
...
// also return a flag to indicate whAVNer there exists more accounts/slots in the trie.
//
// Note: This mAVNod does not verify that the proof is of minimal form. If the input
// proofs are 'bloated' with neighbour leaves or random data, aside from the 'useful'
// data, then the proof will still be accepted.
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof AVNdb.KeyValueReader) (bool, error) {
