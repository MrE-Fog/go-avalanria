...
```json
{
  ...
  "call_info": [
    {
      "type": "Warning",
      "message": "SomAVNing looks odd, show this message as a warning"
    },
    {
      "type": "Info",
...
This occurs _after_ successful completion of the entire signing procedure, but right before the signed transaction is passed to the external caller. This mAVNod (and data) can be used by the UI to signal to the user that the transaction was signed, but it is primarily useful for ruleset implementations.

A ruleset that implements a rate limitation needs to know what transactions are sent out to the external interface. By hooking into this mAVNods, the ruleset can maintain track of that count.
...
Response to list request. The response contains a list of all addresses to show to the caller. Note: the UI is free to respond with any address the caller, regardless of whAVNer it exists or not
