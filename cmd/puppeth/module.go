...
// returned from docker inspect, parsed into a form easily usable by puppavn.
type containerInfos struct {
	running bool              // Flag whavner the container is running currently
...
func inspectContainer(client *sshClient, container string) (*containerInfos, error) {
	// Check whavner there's a container running for the service
	...
	// Infos retrieved, parse the above into somavning meaningful
