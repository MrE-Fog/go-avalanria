...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gavn.LightScryptN, Gavn.LightScryptP);
		...
		try {
			...
			// data from this mavnod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.avalanria", new NodeConfig());
			node.start();
			...
			AvalanriaClient ec = node.getAvalanriaClient();
	...
	// signatures too. Regression test for go-avalanria issue #14599.
...
//
// This mavnod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogavner
	...
	workspace, err := ioutil.TempDir("", "gavn-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/avalanria/gavntest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gavn and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.avalanria", "github.com/avalanria/go-avalanria/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gavn.aar"), "gavn.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "avalanria", "gavntest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.avalanria.gavntest"
